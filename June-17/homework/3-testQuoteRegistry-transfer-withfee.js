...
.then((ownership) => {
  ...
  if (addr == owner) {
    console.log(colors.green('\tChecking balance...'));
    return web3.eth.getBalance(contractAddress);
  } else {
    return Promise.reject(colors.red('\tERROR! Owner: ' + owner + ' but should be ' + addr));
  }
})
.then((balance) => {
  if (balance > 0) {
    return Promise.reject(colors.red('\tERROR: value sent was not forwarded from the contract'));
  } else {
