...
.then((name) => {
  ...
  console.log(colors.green('\tCalling "tranfer" with LOW fee...'));
  var encodedCall = instance.methods.transfer('just another quote', walletA.getChecksumAddressString()).encodeABI();
  var fee = web3.utils.toBN(web3.utils.toWei('0.4', 'ether'));
  return sendTx(walletB, encodedCall, nonceB, fee);
})
.then((tx) => {
...
.then((ownership) => {
  ...
  if (addr == owner) {
    ...
    console.log(colors.red('\tERROR: allowed to transfer ownership with lower fee'));
  }
})
.then((balance) => {
  if (balance > 0) {
    console.log(colors.red('\tERROR: ethers sent got stuck in the contract'));
  } else {
