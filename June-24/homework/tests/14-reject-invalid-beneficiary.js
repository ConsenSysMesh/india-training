...
.then((nonce) => {
  var encoded = instance.methods.reject(wallet.getChecksumAddressString()).encodeABI();
  var tx = buildTx(wallet, encoded, nonce, 0);
  level = 1;
  return web3.eth.sendSignedTransaction(tx);
})
.then((result) => {
  console.log(colors.red('TEST 14 ERROR: Reject should throw if beneficiary is not found'));
})
.catch((ex) => {
  console.log(colors.green('TEST 14 OK!'));
})
