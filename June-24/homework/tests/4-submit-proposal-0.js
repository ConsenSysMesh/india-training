...
var jsonA = fs.readFileSync('p1', 'utf8');
...
.then((price) => {
  ...
  stage = 1;
  var encoded = instance.methods.submitProposal(0).encodeABI();
  return sendTx(walletA, encoded, nonceA, 0);
})
.then((tx) => {
  console.log(colors.red('TEST 4 ERROR: Function was supposed to throw'));
})
.catch((ex) => {
  console.log(colors.green('TEST 4 OK!'));
});
