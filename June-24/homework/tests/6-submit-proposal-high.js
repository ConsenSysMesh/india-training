...
.then((price) => {
  ...
  // contract current holds 2000 szabos
  var proposal = web3.utils.toBN(web3.utils.toWei('201', 'szabo'));
  var encoded = instance.methods.submitProposal(proposal).encodeABI();
  return sendTx(walletA, encoded, nonceA, 0);
})
.then((tx) => {
  console.log(colors.red('TEST 6 ERROR: function did not throw'));
})
.catch((ex) => {
  console.log(colors.green('TEST 6 OK!'));
});
