...
.then((result) => {
  level = 3;
  if (result.length == 1) {
    console.log(colors.green('TEST 22 OK!'));
  } else {
    throw new Error(colors.red('TEST 22 ERROR: Withdraw of remaining value failed'));
  }
})
.catch((ex) => {
  console.log(colors.red('TEST 22 ERROR: Function was not supposed to throw'));
})
