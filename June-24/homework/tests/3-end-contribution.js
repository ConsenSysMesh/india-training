...
var nonceA, nonceB, gasPrice, stage = 0;
...
.then((price) => {
  ...
  stage = 1;
  var encoded = instance.methods.endContributionPeriod().encodeABI();
...
.then((tx) => {
  throw new Error(colors.red('TEST 3 ERROR: function was supposed to thrown'));
})
.catch((ex) => {
  if (stage == 1) {
    console.log(colors.red('TEST 3 ERROR: Error calling endContributionPeriod'));
  } else if (stage == 2) {
    console.log(colors.green('TEST 3 OK!'));
  }
});
